...
for val in df_concentration.loc["115In"]:
    coeffs = dico_elt_corblancsensib["115In"][
        int(np.array(df_ech.loc["numérotation_dilution"])[a]) - 1   # Choisit le bon coefficient suivant après quel échantillon standardisé l'expérience a été faite
    ]
    ...
    df_concentration.loc["115In", col_name] = coeffs * (val - df_blanc.loc["115In", j])   # Insert la concentration d'indium dans la dataframe
    a += 1
